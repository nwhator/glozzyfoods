        ...
        <div className="container">
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
            ...
            <div className="d-flex align-items-center gap-3">
              <Link to="/order-tracking" className="d-none d-md-inline text-white opacity-75 hover-opacity-100">
                <i className="fa-solid fa-truck-fast me-1 text-warning"></i> Track Order
              </Link>
              <Link to="/catering" className="d-none d-sm-inline text-white opacity-75 hover-opacity-100">
                <i className="fa-solid fa-cake-candles me-1 text-warning"></i> Event Catering
              </Link>
              <Link to="/admin" className="badge bg-warning text-dark font-weight-bold px-2 py-1">
                <i className="fa-solid fa-shield-halved me-1"></i> Admin Portal
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="container">
          <div className="d-flex align-items-center justify-content-between gap-3">
            ...
            <nav className="d-none d-lg-flex align-items-center gap-1">
              ...
              <Link to="/shop" className="glozzy-nav-link">Full Menu</Link>
              ...
              <Dropdown className="d-inline-block">
                ...
                <Dropdown.Menu className="shadow-lg border-0 mt-2 p-2 rounded-3" style={{ minWidth: "220px" }}>
              ...
              <Link to="/catering" className="glozzy-nav-link">Catering & Events</Link>
              <Link to="/about" className="glozzy-nav-link">About Us</Link>
              <Link to="/contact" className="glozzy-nav-link">Contact</Link>
            </nav>

            {/* Search Bar */}
            <form onSubmit={handleSearch} className="d-none d-md-flex align-items-center flex-grow-1 mx-3" style={{ maxWidth: "300px" }}>
              <div className="input-group">
                <input
                  type="text"
                  className="form-control rounded-start-pill border-end-0 ps-3"
                  placeholder="Search dishes, cakes, soups..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  style={{ fontSize: "13px", height: "42px", borderColor: "#E2E8F0" }}
                />
                <button
                  type="submit"
                  className="btn btn-outline-secondary rounded-end-pill border-start-0 pe-3 bg-white"
                  style={{ borderColor: "#E2E8F0", color: "#C5161D" }}
                >
            ...
            <div className="d-flex align-items-center gap-2">
              ...
              {currentUser ? (
                <Dropdown className="d-inline-block">
                  <Dropdown.Toggle as="button" className="btn btn-light rounded-pill d-flex align-items-center gap-2 py-2 px-3 border">
                    <i className="fa-solid fa-circle-user text-danger"></i>
                    <span className="d-none d-sm-inline fw-semibold" style={{ fontSize: "13px" }}>
                      {currentUser.name?.split(" ")[0]}
                    </span>
                  </Dropdown.Toggle>
                  <Dropdown.Menu align="end" className="shadow-lg border-0 mt-2 p-2 rounded-3" style={{ minWidth: "200px" }}>
                    <div className="px-3 py-2 border-bottom mb-2">
                      <p className="fw-bold mb-0 text-dark">{currentUser.name}</p>
                      <small className="text-muted">{currentUser.email}</small>
                    </div>
                    <Dropdown.Item as={Link} to="/account" className="py-2">
                      <i className="fa-solid fa-user me-2 text-muted"></i> My Account & Profile
                    </Dropdown.Item>
                    <Dropdown.Item as={Link} to="/account" className="py-2">
                      <i className="fa-solid fa-receipt me-2 text-muted"></i> My Orders History
                    </Dropdown.Item>
                    {isAdmin && (
                ...
                <Link to="/login" className="btn btn-outline-danger rounded-pill px-3 py-2 fw-semibold d-none d-sm-inline-flex" style={{ fontSize: "14px" }}>
          ...
          {mobileMenuOpen && (
            <div className="d-lg-none mt-3 pt-3 border-top">
              ...
              <div className="d-flex flex-column gap-2">
                <Link to="/" onClick={() => setMobileMenuOpen(false)} className="py-2 px-3 rounded-2 fw-semibold text-dark hover-bg-light">
                  <i className="fa-solid fa-house me-2 text-danger"></i> Home
                </Link>
                <Link to="/shop" onClick={() => setMobileMenuOpen(false)} className="py-2 px-3 rounded-2 fw-semibold text-dark hover-bg-light">
                  <i className="fa-solid fa-utensils me-2 text-danger"></i> Full Menu
                </Link>
                <Link to="/catering" onClick={() => setMobileMenuOpen(false)} className="py-2 px-3 rounded-2 fw-semibold text-dark hover-bg-light">
                  <i className="fa-solid fa-champagne-glasses me-2 text-danger"></i> Event Catering
                </Link>
                <Link to="/order-tracking" onClick={() => setMobileMenuOpen(false)} className="py-2 px-3 rounded-2 fw-semibold text-dark hover-bg-light">
                  <i className="fa-solid fa-truck-fast me-2 text-danger"></i> Track Order
                </Link>
                <Link to="/about" onClick={() => setMobileMenuOpen(false)} className="py-2 px-3 rounded-2 fw-semibold text-dark hover-bg-light">
                  <i className="fa-solid fa-circle-info me-2 text-danger"></i> About Us
                </Link>
                <Link to="/contact" onClick={() => setMobileMenuOpen(false)} className="py-2 px-3 rounded-2 fw-semibold text-dark hover-bg-light">
                  <i className="fa-solid fa-phone me-2 text-danger"></i> Contact GlozzyFoods
                </Link>
                {!currentUser && (
